...
$.get(base_api + 'corretor/pesquisar', function(items) {
        ...
        item_html = `
            <tr>
                ...
                <td style="width:100px;">
                    <a href="${base}corretores/salvar/?id=${item.id}" class="btn btn-xs btn-block btn-warning"><i class="fas fa-edit"></i> Editar</a>
...
$(document).on('click', '[data-action]', function(e) {
    ...
    switch(action) {
        case 'delete':
            let confirm = window.confirm('Tem certeza que deseja apagar este corretor?');

            if(confirm) {
                $.ajax({
                    url: base_api + 'corretor/apagar/?id=' + id,
                    type: 'DELETE',
