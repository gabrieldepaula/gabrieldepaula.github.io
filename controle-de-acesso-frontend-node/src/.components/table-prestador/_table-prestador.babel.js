...
$.get(base_api + 'prestadorservico/pesquisar', function(items) {
        ...
        item_html = `
            <tr>
                ...
                <td style="width:100px;">
                    <a href="${base}prestadores-de-servico/salvar/?id=${item.id}" class="btn btn-xs btn-block btn-warning"><i class="fas fa-edit"></i> Editar</a>
...
$(document).on('click', '[data-action]', function(e) {
    ...
    switch(action) {
        case 'delete':
            let confirm = window.confirm('Tem certeza que deseja apagar este prestador de serviço?');

            if(confirm) {
                $.ajax({
                    url: base_api + 'prestadorservico/apagar/?id=' + id,
                    type: 'DELETE',
