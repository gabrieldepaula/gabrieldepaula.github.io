...
if(id) {
    ...
    $.get(base_api + 'corretor/pesquisar', function(items) {
        ...
        $items.each(function(itemIndex, item) {
            if(id == item.id) {
                $el.find('.page-title').text('Editar corretor #' + id + ' - ' + item.nome);
...
$form.on('submit', function(e) {
    ...
    if(formValidated) {
        $.ajax({
            ...
            success: function() {
                alert('Corretor ' + (id ? 'atualizado' : 'cadastrado') + ' com sucesso.');
                window.location.href = base + 'corretores/';
            },
