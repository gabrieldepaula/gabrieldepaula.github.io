...
if(id) {
    ...
    $.get(base_api + 'entregador/pesquisar', function(items) {
        ...
        $items.each(function(itemIndex, item) {
            if(id == item.id) {
                $el.find('.page-title').text('Editar entregador #' + id + ' - ' + item.nome);
...
$form.on('submit', function(e) {
    ...
    if(formValidated) {
        $.ajax({
            url: base_api + 'entregador/salvar',
            type: 'POST',
            dataType: 'json',
            ...
            success: function() {
                alert('Entregador ' + (id ? 'atualizado' : 'cadastrado') + ' com sucesso.');
                window.location.href = base + 'entregadores/';
            },
